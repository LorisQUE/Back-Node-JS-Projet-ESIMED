...
module.exports = class ItemDAO extends BaseDAO {
    ...
    getAllFromPartageList(listId, userid) {
        return new Promise((resolve, reject) =>
            this.db.query("SELECT item.id, quantite, item.label, ischecked, listid, user_id, droit FROM item INNER JOIN list ON listid = list.id INNER JOIN partage ON listid = list_id WHERE listid = $1 AND user_id = $2",[listId, userid])
                .then(res => resolve(res.rows))
                .catch(e => reject(e)))
    };
