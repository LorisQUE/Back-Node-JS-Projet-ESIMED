...
module.exports = class PartageDAO extends BaseDAO {
    ...
    getAllBySharedUser(user) {
        return new Promise((resolve, reject) =>
            this.db.query("SELECT * FROM partage INNER JOIN list ON list.id = list_id WHERE user_id = $1 ORDER BY list_id", [user.id])
    ...
    getAllByList(list) {
        return new Promise((resolve, reject) =>
            this.db.query("SELECT * FROM partage INNER JOIN useraccount ON useraccount.id = user_id WHERE list_id = $1 ORDER BY list_id", [list.id])
                .then(res => resolve(res.rows))
                .catch(e => reject(e)));
    };
